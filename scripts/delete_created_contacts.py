...
def main():
    ...
    # Update history file with only those that failed to delete (non-404 errors)
    with open(cfg.history_file, "w", encoding="utf-8") as f:
        json.dump(remaining_history, f, indent=2)

    # Reset pending updates completely
    if os.path.exists(cfg.pending_update_file):
        with open(cfg.pending_update_file, "w", encoding="utf-8") as f:
            json.dump([], f, indent=2)
        print("Pending updates list has been reset.")

    print(f"Cleanup complete. {len(remaining_history)} contacts remaining in history.")
